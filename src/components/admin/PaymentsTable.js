...
const columns = [
  ...
  { id: 'idno', label: 'ID Number', minWidth: 170, align: 'center' },
  { id: 'receipt_no', label: 'Receipt No', minWidth: 170, align: 'center' },
  { id: 'amount', label: 'Amount', minWidth: 170, align: 'center' },
  { id: 'created_at', label: 'Date', minWidth: 170, align: 'center' },
];
...
const PaymentsTable = () => {
  ...
  const fullScreen = useMediaQuery(theme.breakpoints.down('md'));
    ...
    const fetchDataOnMount = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:5000/get_payments?category=${selectedCategory}&searchstring=${searchString}&page=${currentPage}`,
          {
    ...
    const fetchUsers = async (page) => {
      try {
        ...
        const authToken = localStorage.getItem("access_token");        
        const response = await axios.get(
          `http://localhost:5000/get_payments?category=${selectedCategory}&searchstring=${searchString}&page=${page}`,
          {
            headers: {Authorization: `Bearer ${authToken}`,},
          }
        );
  ...
  useEffect(() => {
    ...
    const handleDelete = async (userToDelete) => {
      try {
        const authToken = localStorage.getItem("access_token");
        const response = await axios.delete(`http://localhost:5000/deleteuser/${userToDelete.id}`, {
          headers: { Authorization: `Bearer ${authToken}` },
        });
